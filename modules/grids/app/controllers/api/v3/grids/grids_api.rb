        ...
        resources :grids do
          ...
          route_param :id, type: Integer, desc: "Grid ID" do
            after_validation do
              ...
              # === 🟢 INÍCIO DA NOSSA INJEÇÃO ===
              # Apenas checamos o nome. O Serviço decide se precisa limpar e popular.
              if @grid.name&.downcase == 'geral'
                # Log leve para debug
                Rails.logger.info "🔎 [API] Verificando integridade do Board Geral ##{@grid.id}"
                
                # O serviço tem a lógica de abortar se já estiver pronto
                ::Boards::MacroBuilderService.new(user: current_user).call(@grid)
                
                # Sempre recarregamos para garantir que, se houve mudança, o front receba
                @grid.widgets.reload 
              end
              # === 🔴 FIM DA NOSSA INJEÇÃO ===
            end
